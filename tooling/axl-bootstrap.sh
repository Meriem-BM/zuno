#!/usr/bin/env bash
# Bootstrap 4 local AXL nodes (one per Zuno role).
#
# For each of cli, watcher, planner, risk this writes:
#   tooling/axl/<role>.pem           ed25519 private key
#   tooling/axl/<role>-config.json   AXL node config (api_port + tcp_port)
#
# Then prints the .env block with the four ZUNO_AXL_<ROLE>_PEER_ID values
...
env_block=()
for entry in "cli 9002 7001" "watcher 9012 7011" "planner 9022 7021" "risk 9032 7031"; do
  set -- $entry
  role=$1
  api_port=$2
  tcp_port=$3
...
echo "ZUNO_AXL_WATCHER_API_URL=http://127.0.0.1:9012"
echo "ZUNO_AXL_PLANNER_API_URL=http://127.0.0.1:9022"
echo "ZUNO_AXL_RISK_API_URL=http://127.0.0.1:9032"
