...
v0.1 supports read-only wallet sessions, Uniswap v3 NFT position reads through
viem, deterministic recommendation, plan storage, diff, simulation preview,
and guarded apply preparation. The full Watcher → Planner → Risk path runs
over AXL when the local mesh is online; otherwise the same deterministic
planner is used locally. `apply` requires explicit wallet-mode signing and
does not store private keys or submit hidden transactions from the terminal.
