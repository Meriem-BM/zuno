...
- Output includes a quick range indicator. Use `inspect position <id>` for
  the full snapshot.
- Position reads are direct viem reads. Recommendations use Watcher → Planner
  → Risk over AXL when the mesh is online.
- The terminal never receives or stores raw private keys.
